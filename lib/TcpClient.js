...
class TcpClient extends BaseClient {
  ...
  constructor(...args) {
    ...
    socket.on('data', (message) => {
      ...
      try {
        // read the message
        // if it is unreadable then assume it is incomplete
        // append current message to the buffer
        // if the message is readable then the buffer is cleared
        // previous messages are ignored
        res = BaseClient.decodeMessage(message);
      } catch (err) {
        try {
          if (buffer !== message) {
            res = BaseClient.decodeMessage(buffer);
          }
          else {
            return;
          }
