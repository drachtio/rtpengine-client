...
class BaseClient extends Emitter {
  ...
  send_internal(name, opts, callback) {
    ...
    const __x = (opts, callback) => {
      ...
      if (this.type === 'udp') {
        this.send(message, this.remotePort, this.remoteHost, (err) => {
          if (err) {
            debug(`error sending command to rtpengine over udp at ${this.remoteHost}:${this.remotePort}`);
  ...
  _onMessageTimeout(id) {
    ...
    callback(new RtpEngineError('rtpengine timeout'));
  }
  ...
  close() {
    ...
    else if ('udp' === this.type) this.client.close();
