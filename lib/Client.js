...
class Client extends BaseClient {
  ...
  _onListening() {
    const recvBufSize = process.env.UDP_RECV_BUFSIZE || process.env.UDP_BUFSIZE;
    const sendBufSize = process.env.UDP_SEND_BUFSIZE || process.env.UDP_BUFSIZE;
    if (recvBufSize) this.socket.setRecvBufferSize(recvBufSize);
    if (sendBufSize) this.socket.setSendBufferSize(sendBufSize);
    super._onListening();
  }
}
